# nohup python -u 仅保留journal_artical.py > 测试openAlex中的doctype.log 2>&1 &
# 测试了一下，发现意义不大，没有标出社论这种东西。我们用参考文献数量来判断，希望参考文献数量>4
# 见：casJournalRefLargerThan4.py
...
with open(mag_dir+'Papers.txt', encoding="ISO-8859-1") as FileObj:
    for lines in tqdm(FileObj):
        ...
        try:
            if temp[13] in journal_set:
                if '2000'< temp[9] <'2022':
                    # print(temp[26],temp[4],temp[3])
                    dst_counter[temp[26]] += 1
                    genre_counter[temp[4]] += 1
                    DocTypeCounter[temp[3]] += 1
                    n_count -= 1
                    if n_count == 0:
                        break
        except:
            pass
print('测试了100000篇00年至22年的结果后：')
print('dst_counter',dst_counter)
print('genre_counter',genre_counter)
print('DocTypeCounter',DocTypeCounter)